		...
		<div class="row justify-content-center">
			...
			<div class="col-md-4 mb-3">
				<div class="category-title">
					<a href="<?php echo site_url('home/filter_listings?category='.slugify($category['name']).'&&amenity=&&video=0'); ?>" style="color: unset;"><?php echo $category['name']; ?> <small style='font-size: 12px;'>(<?php echo count($this->frontend_model->get_category_wise_listings($category['id'])); ?>)</small></a>
				</div>
				<?php
				$sub_categories = $this->crud_model->get_sub_categories($category['id']);
				foreach ($sub_categories->result_array() as $key => $sub_category): ?>
				<a href="<?php echo site_url('home/filter_listings?category='.slugify($sub_category['name']).'&&amenity=&&video=0'); ?>" class="sub-category-link">
